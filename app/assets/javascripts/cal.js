  ...
  for (var i = 0; i < row; i++) {
    ...
    for (var j = 0; j < week.length; j++) {
      if (i == 0 && j < startDayOfWeek) {
        // 1行目で1日まで先月の日付を設定
        calendar += `<td class='disabled' onClick='sendDay(${year}, ${(month + 1)}, ${lastMonthEndDate - startDayOfWeek + j + 1})'
        >` + (lastMonthEndDate - startDayOfWeek + j + 1) + `</td>`;
      } else if (count >= endDate) {
        // 最終行で最終日以降、翌月の日付を設定
        count++;
        calendar += `<td class='disabled' onClick='sendDay(${year}, ${(month + 1)}, ${count - endDate})'
        >` + (count - endDate) + `</td>`;
      } else {
        // 当月の日付を曜日に照らし合わせて設定
        count++;
        if (year == today.getFullYear()
          ...
          calendar += `<td onClick='sendDay(${year}, ${(month + 1)}, ${ count })'
          >` + count + `</td>`;
        }
      }
    }
